#!/usr/bin/env python3
"""Route plain implementation requests into AETHER's native creator lane.

V2 fixes the CLI insertion logic for command handlers implemented as an
if/elif chain. It also adds completion-truth guidance to the creator protocol
so placeholders and stubs are not reported as finished work.

This patch does not change OPEN_SYSTEM, llama.cpp middleware, the active model
checkpoint, executor capabilities, path scopes, or receipt behavior.
"""
...
ROUTE_MARKER = "# AETHER_CREATOR_AUTO_ROUTE_V2"
...
def build_engine(source: str) -> str:
    ...
    if TRUTH_MARKER not in candidate:
        if LOCAL_PROTOCOL_MARKER not in candidate:
            raise RuntimeError("LOCAL_CREATOR_PROTOCOL_MARKER_MISSING")
        if candidate.count(TRUTH_ANCHOR) != 1:
            raise RuntimeError(
                f"CREATOR_TRUTH_ANCHOR_COUNT:{candidate.count(TRUTH_ANCHOR)}"
            )
        candidate = candidate.replace(TRUTH_ANCHOR, TRUTH_ANCHOR + TRUTH_EXTRA, 1)
...
def build_cli(source: str) -> tuple[str, str]:
    ...
    if stripped.startswith("elif "):
        ...
        keyword = "if"
        layout = "standalone-if"
    else:
        raise RuntimeError("CODE_HANDLER_UNSUPPORTED_LAYOUT")

    block = (
        f'{indent}{keyword} _looks_like_creator_request(message):\n'
...
def show_plan() -> int:
    ...
    print("AETHER CREATOR // AUTO ROUTE V2 PLAN")
    ...
    print("CHAINED_IF_ELIF_LAYOUT=SUPPORTED")
...
def apply() -> int:
    ...
    if TRUTH_MARKER in engine_source and ROUTE_MARKER in cli_source:
    ...
    if sha256(ENGINE) != EXPECTED_ENGINE_SHA256:
        ...
        print(f"ACTUAL={sha256(ENGINE)}")
        return 4
    if sha256(CLI) != EXPECTED_CLI_SHA256:
        print("STOP=UNEXPECTED_CLI_SHA256")
        print(f"ACTUAL={sha256(CLI)}")
        return 5
    ...
    try:
        ...
        cli_candidate, handler_layout = build_cli(cli_source)
    ...
    print("AETHER_CREATOR_AUTO_ROUTE=V2")
    print(f"CLI_HANDLER_LAYOUT={handler_layout}")
    print("PLAIN_IMPLEMENTATION_REQUEST=CREATOR_ROUTE")
