...
echo "$STAMP | JUTSUBOYZZ | START" >> "$PIPE/LOGS/pipeline.log"
...
if [ "$VALIDATION" != "ok" ]; then
    echo "$STAMP | JUTSUBOYZZ | REJECT | $name | $VALIDATION" >> "$PIPE/LOGS/pipeline.log"
    jq --arg stamp "$STAMP" --arg pipe "JUTSUBOYZZ" --arg reason "$VALIDATION" --arg severity "$SEVERITY" \
  ...
  echo "$STAMP | JUTSUBOYZZ | PROCESS | $name | TYPE=$TYPE | DEST=$DEST" >> "$PIPE/LOGS/pipeline.log"

  cp "$job" "$PIPE/PROCESS/$name"

  jq --arg stamp "$STAMP" --arg pipe "JUTSUBOYZZ" --arg dest "$DEST" \
    '.status="processed" | .processed_at=$stamp | .pipeline=$pipe | .routed_to=$dest' \
    "$job" > "$PIPE/OUTPUT/$DEST/${name%.json}.processed.json"

  echo "$STAMP | JUTSUBOYZZ | OUTPUT | $DEST/${name%.json}.processed.json" >> "$PIPE/LOGS/pipeline.log"

  mv "$job" "$PIPE/PROCESS/${name%.json}.done.json"
done

echo "$STAMP | JUTSUBOYZZ | END" >> "$PIPE/LOGS/pipeline.log"
echo "JUTSUBOYZZ PIPELINE EXECUTED"
